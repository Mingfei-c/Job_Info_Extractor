...
class JobEmbedding(Base):
    ...
    embedding = Column(Vector(EMBEDDING_DIM)) if PGVECTOR_AVAILABLE else None  # 768-dim vector
...
class EmbeddingService:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise ImportError("langchain-google-genai required: pip install langchain-google-genai")
    ...
    def embed_all_jobs(self, batch_size: int = 50) -> dict:
        ...
        for i in range(0, len(pending_jobs), batch_size):
            ...
            # Progress log
            logger.info(f"Progress: {processed + failed}/{len(pending_jobs)}, "
        ...
        logger.info("=" * 60)
        logger.info(f"Embedding complete: Processed {processed}, Skipped {len(existing_ids)}, Failed {failed}")
    ...
    def search_similar_jobs(self, query_text: str, top_k: int = 10) -> list[dict]:
        ...
        for row in result:
            jobs.append({
                ...
                "description": row[8],
                "redirect_url": row[9],
                "full_description": row[10],  # From full_descriptions table
    ...
    def get_stats(self) -> dict:
        ...
        embedded_jobs = self.db.execute(
            text("SELECT COUNT(*) FROM job_embeddings")
        ).scalar()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Test embedding (only process a few)
        print("\nStarting embedding...")
        result = service.embed_all_jobs(batch_size=10)
        ...
        jobs = service.search_similar_jobs(test_query, top_k=3)
        ...
        print("  pip install langchain-google-genai pgvector")
    except Exception as e:
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
